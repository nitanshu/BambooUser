...
2.  Paths for login, signup, invitation signup or reset-password is only accessible when no logged-in user is present and if requested from a validated session it will redirect to a specified path but <em>(Default is <b>main_app.root_path</b>)</em>.
